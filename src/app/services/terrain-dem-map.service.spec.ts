...
describe('terrain DEM chunks', () => {
  ...
  it('computes a sub-3 km chunk span at mid-latitude', () => {
    const km = demChunkMaxSpanKm(46, DEM_SAMPLE_ZOOM, 1024, 80);
  ...
  it('splits a long leg into enough z15 windows', () => {
    ...
    const chunkKm = demChunkMaxSpanKm(46.5, DEM_SAMPLE_ZOOM, 1024, 80);
  ...
  it('keeps sub-kilometre legs as a single chunk', () => {
    ...
    const chunks = splitSegmentIntoChunks(
      segment,
      demChunkMaxSpanKm(46.5, DEM_SAMPLE_ZOOM, 1024, 80)
    );
